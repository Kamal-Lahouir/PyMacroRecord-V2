...
class EventEditor(Frame):
    ...
    def __init__(self, main_app):
        ...
        columns = ("index", "type", "params", "delay", "label", "comment")
        self.tree = Treeview(
        ...
        self.tree.heading("label", text=text.get("column_label", "Label"))
        self.tree.heading("comment", text=text.get("column_comment", "Comment"))
        ...
        self.tree.column("label", width=120, minwidth=80, stretch=False)
        self.tree.column("comment", width=200, minwidth=100, stretch=True)
        ...
        self._context_menu.add_separator()
        self._context_menu.add_command(
            label=editor_text.get("group_events", "Group Selected (Ctrl+G)"),
            command=self._on_group_selected,
        )
        self._context_menu.add_command(
            label=editor_text.get("ungroup_events", "Ungroup (Ctrl+Shift+G)"),
            command=self._on_ungroup_selected,
        )
        ...
        self.tree.bind("<Control-g>", lambda e: self._on_group_selected())
        self.tree.bind("<Control-Shift-G>", lambda e: self._on_ungroup_selected())
    ...
    def refresh(self):
        ...
        for item in display_items:
            if item["kind"] == "single":
                ...
                tags = self._get_event_tags(event)
                self.tree.insert(
        ...
        # Refresh label navigation dropdown
        toolbar = getattr(self.main_app, "toolbar", None)
        if toolbar:
            toolbar.refresh_label_list()
    ...
    def refresh_row(self, index):
        ...
        # Refresh label navigation dropdown in case label changed
        toolbar = getattr(self.main_app, "toolbar", None)
        if toolbar:
            toolbar.refresh_label_list()
    ...
    def _get_event_tags(self, event):
        """Get all applicable tags for an event (type tag + disabled tag)."""
        tags = []
        type_tag = self._get_event_tag(event.get("type", ""))
        if type_tag:
    ...
    def _event_to_values(self, index, event):
        ...
        type_label = EVENT_TYPE_LABELS.get(etype, etype)

        # Show group name if event is grouped
        group_id = event.get("group_id")
        if group_id and group_id in self.main_app.macro_editor.groups:
            group_name = self.main_app.macro_editor.groups[group_id]["name"]
            type_label = f"[{group_name}] {type_label}"

        params = self._format_params(event)
        delay = f"{event.get('timestamp', 0):.3f}"
        event_label = event.get("label", "")

        # Truncate comment to 50 chars for display
        comment = event.get("comment", "")
        if len(comment) > 50:
            comment = comment[:47] + "..."

        return (index + 1, type_label, params, delay, event_label, comment)
    ...
    def _format_params(event):
        ...
        if t == "cursorMove":
            ...
            timeout = event.get("timeout_label")
            if timeout:
                return f"Jump to '{target}' (timeout: '{timeout}')"
            return f"Jump to '{target}'"
        elif t == "repeat":
            target = event.get("target_label", "?")
            count = event.get("count", 1)
            return f"Loop to '{target}' {count}x"
        elif t == "wait":
            delay = event.get("delay", 0)
            return f"Wait {delay}s"
        return ""
    ...
    def _on_double_click(self, event):
        ...
        # Only allow editing delay (3), label (4), and comment (5) columns
    ...
    def _commit_inline_edit(self):
        ...
        if col == 3:  # delay
            ...
            success, error = self.main_app.macro_editor.update_event_label(index, new_value)
            if not success and error:
                from tkinter import messagebox
                messagebox.showerror("Invalid Label", error)
            self.refresh_row(index)
        elif col == 5:  # comment
            self.main_app.macro_editor.update_event_comment(index, new_value)
            self.refresh_row(index)
    ...
    def _on_group_selected(self):
        """Group the selected events."""
        from windows.main.group_dialog import GroupDialog

        indices = self.get_selected_indices()
        if not indices:
            return

        dialog = GroupDialog(self.main_app, event_indices=indices)
        # Dialog handles creation and refresh

    def _on_ungroup_selected(self):
        """Remove group assignment from selected events."""
        indices = self.get_selected_indices()
        if not indices:
            return

        self.main_app.macro_editor.ungroup_events(indices)
        self.refresh()
