...
class InsertEventDialog(Popup):
    ...
    def __init__(self, main_app, insert_index):
        ...
        # Control flow fields
        self._target_label_var = StringVar(value="")
        self._timeout_label_var = StringVar(value="")
        self._repeat_count_var = StringVar(value="1")
        self._wait_delay_var = StringVar(value="1.0")
    ...
    def _show_fields_for_type(self, event_type):
        ...
        # Control flow event fields
        if event_type == "goto":
            ltl = Label(self._fields_frame, text="Target Label:")
            ltl.grid(row=row, column=0, sticky=W, padx=4, pady=2)
            etl = Entry(self._fields_frame, textvariable=self._target_label_var, width=20)
            etl.grid(row=row, column=1, columnspan=3, sticky=W, padx=4, pady=2)
            self._field_widgets.extend([ltl, etl])
            row += 1

            ltol = Label(self._fields_frame, text="Timeout Label (optional):")
            ltol.grid(row=row, column=0, sticky=W, padx=4, pady=2)
            etol = Entry(self._fields_frame, textvariable=self._timeout_label_var, width=20)
            etol.grid(row=row, column=1, columnspan=3, sticky=W, padx=4, pady=2)
            self._field_widgets.extend([ltol, etol])
            row += 1
            ...
            lrc = Label(self._fields_frame, text="Repeat Count:")
            lrc.grid(row=row, column=0, sticky=W, padx=4, pady=2)
            erc = Entry(self._fields_frame, textvariable=self._repeat_count_var, width=10)
            erc.grid(row=row, column=1, sticky=W, padx=4, pady=2)
            self._field_widgets.extend([lrc, erc])
            row += 1

        elif event_type == "wait":
            lwd = Label(self._fields_frame, text="Wait Duration (s):")
            lwd.grid(row=row, column=0, sticky=W, padx=4, pady=2)
            ewd = Entry(self._fields_frame, textvariable=self._wait_delay_var, width=10)
            ewd.grid(row=row, column=1, sticky=W, padx=4, pady=2)
            self._field_widgets.extend([lwd, ewd])
            row += 1

        # Delay/timestamp is shown for regular events (not control flow)
        if event_type not in ("goto", "repeat", "wait"):
    ...
    def _confirm(self):
        ...
        if event_type == "goto":
            event["target_label"] = self._target_label_var.get().strip()
            timeout = self._timeout_label_var.get().strip()
            if timeout:
                event["timeout_label"] = timeout
            event["timestamp"] = 0  # Control flow events have no delay

        elif event_type == "repeat":
            event["target_label"] = self._target_label_var.get().strip()
            try:
                event["count"] = max(1, int(self._repeat_count_var.get()))
            except ValueError:
                event["count"] = 1
            event["timestamp"] = 0  # Control flow events have no delay
            ...
            try:
                event["delay"] = max(0, float(self._wait_delay_var.get()))
            except ValueError:
                event["delay"] = 1.0
            event["timestamp"] = 0  # Control flow events have no delay
