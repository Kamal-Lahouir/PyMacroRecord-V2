...
class Macro:
    ...
    def __play_events(self):
        ...
        # Track repeat loop counters for control flow
        repeat_loop_counters = {}  # {label: remaining_count}

        while self.playback and (is_infinite or repeat_count < repeat_times):
            event_index = 0
            num_events = len(self.macro_events["events"])

            while event_index < num_events and self.playback:
                events = event_index  # Keep old variable name for compatibility
                ...
                event = self.macro_events["events"][events]
                ...
                # TODO: Handle breakpoints (pause playback)
                # if event.get("breakpoint", False):
                #     # Pause and wait for user to continue
                #     pass

                # Handle control flow events BEFORE processing delays
                if event_type == "goto":
                    # Jump to labeled event
                    target_label = event.get("target_label")
                    if target_label:
                        target_index = self._find_event_by_label(target_label)
                        if target_index is not None:
                            event_index = target_index
                            continue
                        else:
                            # Label not found, try timeout_label if present
                            timeout_label = event.get("timeout_label")
                            if timeout_label:
                                timeout_index = self._find_event_by_label(timeout_label)
                                if timeout_index is not None:
                                    event_index = timeout_index
                                    continue
                    # If no valid target, just continue to next event
                    event_index += 1
                    continue

                elif event_type == "repeat":
                    # Loop back to labeled event N times
                    target_label = event.get("target_label")
                    count = event.get("count", 1)
                    if target_label:
                        # Use event index as unique key for this repeat statement
                        loop_key = f"{events}_{target_label}"

                        if loop_key not in repeat_loop_counters:
                            # First time hitting this repeat - initialize counter
                            repeat_loop_counters[loop_key] = count - 1
                            # Jump to target
                            target_index = self._find_event_by_label(target_label)
                            if target_index is not None:
                                event_index = target_index
                                continue
                        elif repeat_loop_counters[loop_key] > 0:
                            # Still have loops remaining
                            repeat_loop_counters[loop_key] -= 1
                            # Jump to target
                            target_index = self._find_event_by_label(target_label)
                            if target_index is not None:
                                event_index = target_index
                            ...
                            # Loop finished, remove from tracking and continue
                            del repeat_loop_counters[loop_key]
                    event_index += 1
                    continue
                ...
                # Normal event processing with delay
                ...
                if event_type == "cursorMove":  # Cursor Move
                    ...
                    if event["key"] is not None:
                        try:
                            if "Key." not in event["key"]:
                                keyToPress = event["key"]
                            else:
                                keyToPress = eval(event["key"])
            ...
            # Clear repeat loop counters for this playthrough
            repeat_loop_counters.clear()
    ...
    def _find_event_by_label(self, label):
        """Find the index of the first event with the given label.

        Returns:
            int or None: Event index if found, None otherwise
        """
        for i, event in enumerate(self.macro_events["events"]):
